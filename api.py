...
def configure_ngrok():
    ngrok_token = "2pFnHZatXkkyCCzbIb6azuhUC7S_23urQS6nGXq4Un9ekJzmk"
    try:
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("anony.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated anony.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update anony.txt on VPS: {str(e)}")
